...
namespace AE_ACR.GLA.SlotResolvers
{
    public class DK_Ability_精雕怒斩 : ISlotResolver
    {
        public int Check()
        {
            ...
            if (Core.Resolve<MemApiSpell>().CheckActionChange(DKData.精雕怒斩CarveAndSpit).IsReady() == false)
            {
                return -3;
            }
            ...
            if (Core.Resolve<MemApiSpell>().CheckActionChange(DKData.精雕怒斩CarveAndSpit).IsReady() == true)
            {
                return 0;
            }
        ...
        {
            Spell spell = Core.Resolve<MemApiSpell>().CheckActionChange(DKData.精雕怒斩CarveAndSpit).GetSpell();
