...
internal class TriggerAction_UI_QT : ITriggerAction
{
    public string DisplayName { get; } = "DK/QT_V2";
    public string? Remark { get; set; }

    public Dictionary<string, bool> qtValues = new();

    private string[]? qtArray = DRKRotationEntry.QT?.GetQtArray();

    public TriggerAction_UI_QT()
    {
        qtArray = DRKRotationEntry.QT.GetQtArray();
    }
    ...
    public bool Handle()
    {
        foreach (var kvp in qtValues)
        {
            if (DRKRotationEntry.QT != null)
                DRKRotationEntry.QT.SetQt(kvp.Key, kvp.Value);
        }
        return true;
    }
}