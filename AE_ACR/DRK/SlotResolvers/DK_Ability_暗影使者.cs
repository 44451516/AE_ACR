...
namespace AE_ACR.DRK.SlotResolvers
{
	public class DK_Ability_暗影使者 : ISlotResolver
	{
		...
		public int Check()
		{
			...
			if (LastSpell == DKData.Shadowbringer暗影使者)
			{
				return -1;
			}
			...
			if (Core.Resolve<MemApiSpell>().CheckActionChange(DKData.Shadowbringer暗影使者).IsReady() == true)
			{
				return 0;
			}
		...
		{
			Spell spell = Core.Resolve<MemApiSpell>().CheckActionChange(DKData.Shadowbringer暗影使者).GetSpell();
