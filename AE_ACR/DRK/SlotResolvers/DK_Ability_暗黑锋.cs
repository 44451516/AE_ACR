...
public class DK_Ability_暗黑锋 : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (LastSpell == Core.Resolve<MemApiSpell>().CheckActionChange(DKData.暗黑锋).GetSpell().Id) return -1;
        ...
        {
            if (darksideTimeRemaining <= 10 * 1000) return 0;
            ...
            if (Core.Me.CurrentMp >= 9800) return 0;
            ...
            if (Core.Me.CurrentMp >= 8600 && LastBaseGcd == DKData.单体2SyphonStrike && GameObjectExtension.HasAura(Core.Me, DKData.Buffs.嗜血BloodWeapon)) return 0;

            if (Core.Me.CurrentMp >= 9200 && GameObjectExtension.HasAura(Core.Me, DKData.Buffs.嗜血BloodWeapon)) return 0;
